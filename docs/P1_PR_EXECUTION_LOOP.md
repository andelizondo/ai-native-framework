...
3. **Non-blocking suggestions** remain the merging agent’s responsibility: fix when reasonable; otherwise reply with a clear decision per thread or in one summary comment that enumerates each item using the outcomes in `.coderabbit.yaml`: **fix**, **accept as follow-up**, or **won't change**. “Looks good” without addressing open threads is not sufficient.
