...
- **Configured AI reviewer status context** — repository-configured reviewer status for the current head SHA (currently `CodeRabbit`). Missing, pending, or failing reviewer status is an unconditional blocker. The `p1-policy` `decide` job **polls** while this context is absent or `pending` (bounded by repo variables `P1_POLICY_REVIEWER_STATUS_WAIT_SECONDS` and `P1_POLICY_REVIEWER_STATUS_POLL_INTERVAL_SECONDS` in the workflow) so the check stays **in progress** until the reviewer posts a terminal status, instead of failing red immediately and forcing empty commits.
