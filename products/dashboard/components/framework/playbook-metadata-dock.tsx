...
import {
  ...
  createPlaybookInputAction,
  createPlaybookOutputAction,
  deletePlaybookInputAction,
  deletePlaybookOutputAction,
  listOutputGroupsForOtherPlaybooksAction,
  listPlaybookInputsAction,
  listPlaybookOutputsAction,
  reorderPlaybookInputsAction,
...
function fromServerOutput(output: PlaybookOutput): OutputDraftRow {
  return {
    id: output.id,
    isPending: false,
    name: output.name,
...
function isOutputDirty(row: OutputDraftRow): boolean {
...
export interface PlaybookMetadataDockProps {
  playbookId: string;
  initialInputs?: PlaybookInput[];
  initialOutputs?: PlaybookOutput[];
  /** All playbooks (any type). Used to look up icon/color for the input
   *  row's upstream-playbook avatar. */
...
export function PlaybookMetadataDock({
  playbookId,
  initialInputs,
  initialOutputs,
  playbooks = [],
  ...
  return (
    <>
      ...
      />
      <aside
        data-testid="playbook-metadata-dock"
        aria-label="Playbook metadata"
        className={cn(
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 space-y-4">
          <PlaybookInputsSection
            playbookId={playbookId}
            initialInputs={initialInputs}
            playbooks={playbooks}
          />
          <PlaybookOutputsSection
            playbookId={playbookId}
...
/** Back-compat wrapper. Existing imports/tests still target
 *  `PlaybookOutputsDock`; we render only the Outputs section here so the
 *  legacy outputs-only mount point (and its `playbook-outputs-editor`
 *  test id) keep working without pulling in the Inputs fetch. */
...
interface PlaybookOutputsSectionProps {
  playbookId: string;
  initialOutputs?: PlaybookOutput[];
}

function PlaybookOutputsSection({
  playbookId,
  initialOutputs,
}: PlaybookOutputsSectionProps) {
  const { success: toastSuccess, error: toastError } = useToast();
  const [rows, setRows] = useState<OutputDraftRow[]>(
    () => (initialOutputs ?? []).map(fromServerOutput),
  );
  ...
  useEffect(() => {
    ...
    listPlaybookOutputsAction(playbookId)
      .then((outputs) => {
        if (!active) return;
        setRows(outputs.map(fromServerOutput));
        setLoaded(true);
      })
      .catch((err) => {
        if (!active) return;
  ...
  const setRow = useCallback((id: string, patch: Partial<OutputDraftRow>) => {
    setRows((current) =>
    ...
    async (id: string) => {
      ...
      try {
        if (row.isPending) {
          ...
          setRows((current) =>
            current.map((r) => (r.id === id ? fromServerOutput(created) : r)),
          );
          ...
          setRows((current) =>
            current.map((r) => (r.id === id ? fromServerOutput(updated) : r)),
          );
...
// ---------------------------------------------------------------------------
// Inputs section — wires this playbook to outputs of *other* playbooks. No
// free-form rows; the "+ Add input" affordance opens a dropdown of every
// available upstream `{playbook → output}` pair.
// ---------------------------------------------------------------------------

interface PlaybookInputsSectionProps {
  playbookId: string;
  initialInputs?: PlaybookInput[];
  playbooks: FrameworkItem[];
}

function PlaybookInputsSection({
  playbookId,
  initialInputs,
  playbooks,
}: PlaybookInputsSectionProps) {
  const { success: toastSuccess, error: toastError } = useToast();
  const [rows, setRows] = useState<PlaybookInput[]>(initialInputs ?? []);
  const [loaded, setLoaded] = useState(initialInputs !== undefined);
  const [collapsed, setCollapsed] = useState(false);
  ...
  // Initial fetch: inputs + cross-playbook outputs catalog. Catalog
  // populates the "+ Add input" dropdown.
  useEffect(() => {
    let active = true;
    async function load() {
      try {
        const [inputs, groups] = await Promise.all([
          initialInputs !== undefined
            ? Promise.resolve(initialInputs)
            : listPlaybookInputsAction(playbookId),
          listOutputGroupsForOtherPlaybooksAction(playbookId),
        ]);
        if (!active) return;
        setRows(inputs);
        ...
        if (!active) return;
        toastError(err instanceof Error ? err.message : "Could not load inputs");
        setLoaded(true);
      }
    }
    void load();
  ...
  }, [playbookId, initialInputs, toastError]);
  ...
  const playbookById = useMemo(
    () => new Map(playbooks.map((pb) => [pb.id, pb])),
    [playbooks],
  );

  // Hide upstreams that are already wired so the picker can't double-wire
  // the same output. Groups that lose every output are dropped entirely.
  const availableForAdd = useMemo(() => {
    const taken = new Set(rows.map((r) => r.upstreamOutputId));
    if (taken.size === 0) return outputGroups;
    return outputGroups
      .map((g) => ({ ...g, outputs: g.outputs.filter((o) => !taken.has(o.id)) }))
      .filter((g) => g.outputs.length > 0);
  }, [outputGroups, rows]);

  const handlePick = useCallback(
    async (value: PlaybookOutputPickerValue | null) => {
      if (value === null) return;
      setBusy(true);
      try {
        const created = await createPlaybookInputAction({
          playbookId,
          upstreamOutputId: value.outputId,
        });
        setRows((current) => [...current, created]);
        toastSuccess("Input added");
      } catch (err) {
        toastError(err instanceof Error ? err.message : "Could not add input");
      } finally {
        setBusy(false);
      }
    },
    [playbookId, toastSuccess, toastError],
  );

  const openDeleteConfirm = useCallback((row: PlaybookInput) => {
    setPendingDelete({
      id: row.id,
      label: `${row.upstreamPlaybookName} / ${row.upstreamOutputName}`,
    });
  }, []);
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    setConfirmInFlight(true);
    try {
      await deletePlaybookInputAction(target.id);
      setRows((current) => current.filter((r) => r.id !== target.id));
      toastSuccess("Input removed");
      setPendingDelete(null);
    } catch (err) {
      toastError(err instanceof Error ? err.message : "Delete failed");
    } finally {
  ...
  }, [pendingDelete, toastSuccess, toastError]);
  ...
  return (
    <>
      <section
        className={cn("pb-drawer-sec", "pb-drawer-inputs")}
        data-collapsed={collapsed}
      >
        <div className="pb-drawer-sec__head">
          <button
          ...
          >
            ...
            />
            <span className="pb-drawer-sec__lbl">
              Inputs <span className="pb-drawer-sec__count">{total}</span>
            </span>
          </button>
        </div>
        ...
        {!collapsed ? (
          !loaded ? (
            <ul
              aria-label="Loading inputs"
              data-testid="playbook-inputs-skeleton"
              className="mt-2 flex flex-col gap-2"
            >
        ...
        <ConfirmModal
          ...
          description={
            <div className="space-y-2">
              <p>
                This unlinks{" "}
                <span className="font-medium text-t1">{pendingDelete.label}</span>{" "}
                from this playbook&rsquo;s declared inputs.
              </p>
              <p className="text-t3">
                Tasks already created from this playbook keep their own copy of
                this wiring and are not affected.
              </p>
            </div>
          }
          confirmLabel="Remove"
...
interface SortableInputRowProps {
  row: PlaybookInput;
  upstreamPlaybook: FrameworkItem | undefined;
  onDelete: () => void;
}
    ...
    >
      <div className="flex items-center gap-2">
        <button
          ...
          data-testid={`playbook-input-drag-${row.id}`}
        ...
        <button
          ...
          title="Remove input"
          data-testid={`playbook-input-delete-${row.id}`}
...
interface SortableOutputRowProps {
  ...
  saving: boolean;
  dirty: boolean;
  onChange: (patch: Partial<OutputDraftRow>) => void;
  onSave: () => void;
  onDelete: () => void;
  onDiscard: () => void;
}
