...
- Wait for CodeRabbit's automatic review run by default. If no CodeRabbit comment or status appears after roughly 15 seconds on a new head SHA, you may post `@coderabbitai review` without asking. If CodeRabbit has already posted a "review in progress" style comment or otherwise clearly started, poll for up to 5 minutes in 1-minute intervals and then ask the user before posting a manual `@coderabbitai review` recovery comment. After a **rate limit**, retry `@coderabbitai review` on the **current** head; a green reviewer check does not prove a new **submitted** review replaced an older **`CHANGES_REQUESTED`**.
- When review threads are still **unresolved** after fixes or replies, or when every thread has a visible outcome on the **current** head but **`CHANGES_REQUESTED`** remains without a new submission on that head, post **`@coderabbitai review`** so CodeRabbit can rerun and submit a fresh review (see `ai/skills/developer.md` step 9 and `ai/playbooks/pull-request-execution-loop.md` section 3). Do not use this to skip thread closure or while a review is clearly in progress (respect the wait window above).
