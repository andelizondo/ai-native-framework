...
- Configured AI review (CodeRabbit) is a **soft-mandatory** gate for substance, not only for a green status. Before merge, **every** open review thread from that reviewer **MUST** be resolved by fixing the code **or** by a **visible** reply on the PR. Use the same **canonical decision outcomes** as `.coderabbit.yaml`: **fix** (applied on the head SHA), **accept as follow-up** (brief justification), or **won't change** (rationale). Equivalent wording: *fixed* → **fix**; *deferred* → **accept as follow-up**; *rejected* → **won't change**. Per-thread replies **or** one consolidated comment that **lists each finding** with one of those outcomes is acceptable for audit. Resolving threads in GitHub **only** to satisfy “conversations resolved”, without that accounting, violates repository policy. Reasonable nits **SHOULD** be fixed when effort is small. Thread **state** in GitHub must still end **resolved** or **outdated** in a way that matches the substance—see `docs/P1_PR_EXECUTION_LOOP.md` (finding closure + §1.5).
