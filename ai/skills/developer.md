...
9. **Ask CodeRabbit to clear pending request-changes without a full re-review:** When per-thread outcomes (`fix:` / `accept as follow-up:` / `won't change:`) are on the threads and substance is confirmed on the **current** head, but **`CHANGES_REQUESTED`** persists or threads still need the bot to acknowledge closure, post **one short** PR comment using the **canonical CodeRabbit approval prompt** in `ai/playbooks/pull-request-execution-loop.md` (section 3). **Do not** post `@coderabbitai review` for this—it triggers a **slow full re-review**. Reserve `@coderabbitai review` only for stalled automatic runs (no reviewer signal on a new head, or operator agrees after the in-progress wait in `AGENTS.md`).
