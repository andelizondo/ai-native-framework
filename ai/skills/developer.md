...
9. When **`CHANGES_REQUESTED`** persists after per-thread outcomes are on the threads, post the **canonical approval prompt** from `ai/playbooks/pull-request-execution-loop.md` section 3 item 6. Do **not** post `@coderabbitai review`—it triggers a slow full re-review. Reserve that for stalled automatic runs only.
