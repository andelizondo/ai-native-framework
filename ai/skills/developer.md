...
6. If you push a commit that addresses review findings, post those thread replies in the same work cycle before moving on to waiting for checks, reviewer reruns, or merge decisions. Do not treat "fix committed" as sufficient closure by itself, and do not replace per-thread replies with a consolidated PR comment.
