...
8. When every configured merge gate for the current head is green and review threads are handled per playbook, **complete the merge** (e.g. `gh pr merge` or host UI) if no executor will do it automatically; if the repo uses a merge queue (such as Mergify), **verify** the PR merged once queue conditions are met. Do not stop at “PR opened.” Prefer bounded waits or the operator signaling that checks finished over endless polling.
