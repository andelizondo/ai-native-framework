...
- 2026-04-18: Introduced a 3-tier deployment environment model (production / staging / development) for this repository and `products/dashboard`. Key decisions: (1) `staging` is a long-lived protected branch with identical protection rules to `main`; feature PRs target `staging`, not `main`. (2) `staging` → `main` is the release gate, automated via release-please; no separate code review required when CI is green; merge must use regular merge (not squash) to preserve conventional commit history for release-please version bumping. (3) Nightly CI targets `staging.ai-native-framework.app` via `STAGING_URL` repo variable; production is never a CI target. (4) `NEXT_PUBLIC_POSTHOG_PROJECT_TOKEN` and `NEXT_PUBLIC_POSTHOG_HOST` are scoped to Vercel's Production environment only — PostHog does not initialize on Preview or Development deployments. (5) Sentry remains active on Preview/staging for real error visibility, relying on environment tags for dashboard filtering. New playbook: `ai/playbooks/environment-separation.md` — reusable procedure for applying this pattern to any product.
