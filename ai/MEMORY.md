...
- For this repository, CodeRabbit should be allowed to start automatically; `@coderabbitai review` is only for stalled automatic runs (no reviewer signal after ~15s on a new head, or operator agrees after the in-progress wait). When requested changes are already addressed on the current head, use the **canonical approval prompt** in `ai/playbooks/pull-request-execution-loop.md` section 3 item 6—not `@coderabbitai review` (full re-review is slow).
...
- 2026-04-17: When per-thread outcomes are complete on the current head but **`CHANGES_REQUESTED`** persists, prefer the **canonical approval prompt** (`@coderabbitai` one-liner in `ai/playbooks/pull-request-execution-loop.md` section 3 item 6) over `@coderabbitai review` (slow full re-review).
